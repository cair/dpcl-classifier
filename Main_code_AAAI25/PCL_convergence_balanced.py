...
@jit(nopython=True)
def PCL(n: int, ta_state: int, examples: np.ndarray, labels: np.ndarray,
...
def main(n: int, states: int, epochs: int, clauses: int, runs: int, p: float) -> None:
    ...
    for run_number in range(runs):
        ...
        X_train_0 = examples[labels == 0]
        X_train_1 = examples[labels == 1]
        X_train_balanced = np.zeros((2, n), dtype=np.uint32)
        Y_train_balanced = np.zeros(2, dtype=np.uint32)
        ta_state = np.random.choice(np.array([states, states + 1]), size=(clauses, n, 2)).astype(np.int32)
        for epoch in range(epochs):
            X_train_balanced[0, :] = X_train_1[np.random.randint(X_train_1.shape[0]), :]
            Y_train_balanced[0] = 1
            X_train_balanced[1, :] = X_train_0[np.random.randint(X_train_0.shape[0]), :]
            Y_train_balanced[1] = 0
            result = PCL(n, ta_state, np.array(X_train_balanced), np.array(Y_train_balanced), clauses, states, p)
...
if __name__ == '__main__':
    ...
    for i in range(1, 13):
        ...
        main(i, args.states, 100, args.clauses, args.runs, args.p)
