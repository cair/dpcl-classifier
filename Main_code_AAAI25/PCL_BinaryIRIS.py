...
def run_experiment(states=10000, epochs=1, clauses=150, runs=100, pl=0.6, pu=0.8):
    ...
    for i in range(runs):
        ...
        train = 0.7
        X_train = data[:int(data.shape[0] * train), 0:16]  # Input features
        Y_train = data[:int(data.shape[0] * train), 16]  # Target value
        Y_train[Y_train != 1] = 0


        X_test = data[int(data.shape[0] * train):, 0:16]  # Input features
        Y_test = data[int(data.shape[0] * train):, 16]  # Target value
        Y_test[Y_test != 1] = 0
    ...
    #plt.show()
    return([min(Accuracies), max(Accuracies), np.mean(Accuracies), np.std(Accuracies)])
...
def main():
    ...
    avgs = []
    stds = []
    array_indices = range(clauses)
    for i in array_indices:
        print('---------------#Clause', i, ' -----------------------')
        a = run_experiment(states, epochs, i, runs, pl, pu)
        mins.append(a[0])
        maxs.append(a[1])
        avgs.append(a[2])
        stds.append(a[3])
    ...
    # Plotting
    #plt.plot(array_indices, all, marker='o', linestyle='-', color='b') #, label='Array Values'
    # Adding labels and title
    # plt.xlabel('Number of clauses')
    # plt.ylabel('Accuracy')
    # plt.title('Accuracy w.r.t number of clauses using PCL on Binary IRIS')
    # plt.xticks(range(min(array_indices), max(array_indices) + 1))
    #
    # # Adding legend
    # plt.legend()
    # plt.savefig('IRIS_Clauses.pdf')
    # # Show the plot
    # plt.show()
    ...
    # Plotting
    plt.plot(array_indices, avg_values, marker='o', linestyle='-', color='b', label='Average')
    plt.fill_between(array_indices, min_values, max_values, color='gray', alpha=0.3, label='Min-Max Range')
    plt.errorbar(array_indices, avg_values, yerr=std_values, fmt='none', color='red', ecolor='red', capsize=5,
                 label='Standard Deviation')

    # Adding labels and title
    plt.xlabel('Number of clauses')
    plt.ylabel('Accuracy')
    plt.title('Accuracies statistics on binary IRIS')

    # Adding legend
    plt.legend()
    plt.xticks(range(min(array_indices), max(array_indices) + 1))

    plt.savefig('IRIS_Clause_var_22.pdf')
    # Show the plot
    plt.show()
    ...
    for i in range(100):
        print('---------------#Epochs', i ,' -----------------------')
        run_experiment(states, i, clauses, runs, pl, pu)
        print('---------------#Epochs', i , '-----------------------')
    '''
